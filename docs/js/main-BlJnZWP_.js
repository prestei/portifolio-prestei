(function(){const s=window.PRESTEI_DATA;if(!s)return;const h=`https://wa.me/${s.whatsapp}?text=${encodeURIComponent(s.whatsappMessage)}`;function p(){document.querySelectorAll("[data-whatsapp]").forEach(t=>{t.setAttribute("href",h)})}function u(){const t=document.querySelector("[data-services]");t&&(t.innerHTML=s.services.map(e=>`
      ...
      </article>`).join(""))}function m(t){const e=`Olá! Vim através do site da Prestei e gostaria de saber mais sobre o projeto "${t}".`;return`https://wa.me/${s.whatsapp}?text=${encodeURIComponent(e)}`}function w(){const t=document.querySelector("[data-showcase]");!t||!s.projects.length||(t.innerHTML=s.projects.map((e,a)=>{const r=a%2===1?" showcase-item--reverse":"",n=e.tech.map(i=>`<span>${i}</span>`).join("");return`
      <article class="showcase-item${r}" data-showcase-item>
        ...
        <div class="showcase-item__info">
          ...
          <div class="showcase-item__tech">${n}</div>
          <a class="btn btn--primary" href="${m(e.title)}" target="_blank" rel="noopener noreferrer" data-magnetic>
            Ver Projeto <i data-lucide="arrow-up-right"></i>
          </a>
        </div>
      </article>`}).join(""))}function _(){const t=document.querySelector("[data-skills]"),e=document.querySelector("[data-tech-stage]"),a=s.stack,r=s.skills||[];if(t){const o=a?.mid?.map(d=>d.name)||[],l=o.length?o:r;t.innerHTML=l.map(d=>`<span class="badge"><span class="badge__dot"></span>${d}</span>`).join("")}if(!e||!a)return;const n={React:"Re","Next.js":"Nx",Laravel:"La","Node.js":"No",TypeScript:"TS",PHP:"Ph",Python:"Py",Git:"Git",Angular:"Ng",MongoDB:"Mo",MySQL:"My",CSS3:"Cs",HTML5:"Ht",JavaScript:"JS"},i=(o,l)=>{const d=o.length||1,y=o.map((c,S)=>{const H=360/d*S-90,A=n[c.name]||c.name.slice(0,2);return`
          <div class="tech-sat" style="--a:${H}deg">
            <button
              type="button"
              class="tech-chip"
              data-tech="${c.name}"
            ...
            >
              <span class="tech-chip__face">
                <span class="tech-chip__body">
                  <span class="tech-chip__glyph" aria-hidden="true">${A}</span>
    ...
    `}function g(){document.querySelectorAll("[data-cnpj]").forEach(t=>{t.textContent=`CNPJ ${s.cnpj}`})}function v(){const t=document.getElementById("nav"),e=document.querySelector("[data-nav-toggle]"),a=document.getElementById("mobile-menu"),r=()=>t?.classList.toggle("is-scrolled",window.scrollY>8);if(r(),window.addEventListener("scroll",r,{passive:!0}),e&&a){const n=i=>{a.hidden=!i,a.classList.toggle("is-open",i),e.setAttribute("aria-expanded",String(i)),document.body.style.overflow=i?"hidden":"",window.lucide&&(e.innerHTML=i?'<i data-lucide="x" aria-hidden="true"></i>':'<i data-lucide="menu" aria-hidden="true"></i>',window.lucide.createIcons({nodes:[e]}))};e.addEventListener("click",()=>n(e.getAttribute("aria-expanded")!=="true")),a.querySelectorAll("a").forEach(i=>i.addEventListener("click",()=>n(!1)))}}function f(){document.querySelectorAll('a[href^="#"]').forEach(t=>{t.addEventListener("click",e=>{const a=t.getAttribute("href"),r=a&&document.querySelector(a);r&&(e.preventDefault(),window.scrollTo({top:r.getBoundingClientRect().top+window.scrollY-72,behavior:"smooth"}))})})}function b(){const t=document.querySelector("[data-marquee-track]");if(!t||!s.capabilities?.length)return;const e=(n,i)=>`
      ...
      </span>`,a=s.capabilities.map((n,i)=>e(n,`a-${i}`)).join(""),r=s.capabilities.map((n,i)=>e(n,`b-${i}`)).join("");t.innerHTML=a+r}function $(){b(),u(),w(),_(),g(),p(),v(),f(),document.querySelectorAll("[data-year]").forEach(t=>{t.textContent=String(new Date().getFullYear())}),window.lucide&&window.lucide.createIcons(),requestAnimationFrame(()=>{window.initHeroExperience&&window.initHeroExperience(),window.initMotion&&window.initMotion(),window.initShowcaseMotion&&window.initShowcaseMotion();const t=()=>{window.initThreeHero&&(window.initThreeHero(),document.body.dataset.hero3d="ready"),window.initShowcaseBg&&window.initShowcaseBg()};if(window.initThreeHero&&window.THREE)requestAnimationFrame(t);else{document.body.dataset.hero3d="pending";const e=setInterval(()=>{!window.THREE||!window.initThreeHero||(clearInterval(e),t())},40);setTimeout(()=>clearInterval(e),8e3)}})}window.initPresteiApp=$})();

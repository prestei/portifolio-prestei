(function(){const s=window.PRESTEI_DATA;if(!s)return;const h=`https://wa.me/${s.whatsapp}?text=${encodeURIComponent(s.whatsappMessage)}`;function u(){document.querySelectorAll("[data-whatsapp]").forEach(t=>{t.setAttribute("href",h)})}function p(){const t=document.querySelector("[data-services]");t&&(t.innerHTML=s.services.map(e=>`
      ...
      </article>`).join(""))}function m(){const t=document.querySelector("[data-showcase]");!t||!s.projects.length||(t.innerHTML=s.projects.map((e,a)=>`
      ...
      </article>`).join(""))}function w(){const t=document.querySelector("[data-skills]"),e=document.querySelector("[data-tech-stage]"),a=s.stack,r=s.skills||[];if(t){const i=a?.mid?.map(c=>c.name)||[],d=(i.length?i:r).map(c=>`<span class="badge"><span class="badge__dot"></span>${c}</span>`).join("");t.innerHTML=`<div class="tech__cloud-track" aria-hidden="true">${d}${d}</div>`}if(!e||!a)return;const n=(i,l)=>{const d=i.length||1,c=i.map((o,$)=>`
          <div class="tech-sat" style="--a:${360/d*$-90}deg">
            <button
              type="button"
              class="tech-chip"
              data-tech="${o.name}"
